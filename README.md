...
> 基于 LLM 驱动的多智能体协同框架，覆盖**全市场筛选 → 新闻分析 → Alpha 挖掘 → 风险控制 → 组合优化 → 回测执行 → 纸交易**的全自动量化投研流水线。支持**美股 (Alpaca) + A 股 (Tushare/Akshare/EMT)** 双市场，集成 **Neo4j 图记忆（自我优化）** + **ChromaDB RAG 知识库** + **CCTV 板块趋势分析**，实现策略的持续学习与进化。

> 最新回测 (2026.05-06, A股小盘≤300亿): **+39.6% 收益, Sharpe 10.2, 最大回撤 -3.1%**
...
```text
                ...
                ▼                   ▼                   ▼
    ┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐
    │  News Agent     │ │  Alpha Agent    │ │  Risk Agent     │
    │  (新闻情绪)      │ │  (信号生成)      │ │  (多维风控)      │
    │                 │ │                 │ │                 │
    │ • CCTV 板块趋势 │ │ • Alpha158 因子 │ │ • 6维度综合评分  │
    │ • 9板块分组分析 │ │ • LightGBM+RF   │ │ • 个股风险画像   │
    │ • 情感词典打分  │ │   +Ridge Ensemble│ │ • 仓位上限计算   │
    │ • 新闻↔信号融合 │ │ • 横截面排名    │ │ • 市场状态识别   │
    └────────┬────────┘ └────────┬────────┘ └────────┬────────┘
             │                   │                   │
             └───────────────────┼───────────────────┘
                                 ▼
              ...
              ▼                ▼                ▼
    ┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐
    │ Backtest Agent  │ │ Execution Agent │ │  Memory System  │
    │  (回测评估)      │ │  (订单执行)      │ │  (自我优化)      │
    │                 │ │                 │ │                 │
    │ • 价值核算回测  │ │ • Alpaca 纸交易  │ │ • Neo4j 图记忆   │
    │ • 滚动周验证    │ │ • CNPaperBroker  │ │ • 历史经验召回   │
    │ • 绩效归因分析  │ │ • EMT 极速柜台   │ │ • 仓位自动调整   │
    │ • 实时指标      │ │ • T+1 / 涨跌停   │ │ • ChromaDB RAG   │
    └─────────────────┘ └─────────────────┘ └─────────────────┘
```
...
| `--no-filter` | 关闭全市场预筛选 |
| `--max-market-cap 300` | **市值上限**（亿元）。300 = 只选 ≤300亿 小盘 |
| `--max-price 50` | **股价上限**（¥）。50 = 只选 ≤¥50 的股票 |
| `--max-universe 200` | 最大候选股票数（默认 200） |
| `--max-positions 20` | 最大持仓数（默认 20） |
| `--capital 100000` | 初始资金（¥，默认 100,000） |

### 4.7 记忆与新闻 CLI 参数

| 参数 | 说明 |
|------|------|
| `--no-memory` | 关闭 Neo4j 图记忆（默认开启） |
| `--no-rag` | 关闭 ChromaDB 知识库检索 |
| `--news` / `--no-news` | 开启/关闭新闻情绪分析（默认开启） |
| `--no-news` | 关闭 CCTV 板块趋势 + 个股新闻分析 |
...
```bash
# 小盘低价策略（市值≤300亿、单价≤50元）
python run_paper_trading.py --market cn --universe liquid_cn \
  --max-market-cap 300 --max-price 50 \
  --max-universe 40 --capital 100000 \
  --mode backtest --start 2026-05-01 --end 2026-06-04

# 实时交易（开盘时间运行）
python run_paper_trading.py --market cn --universe liquid_cn \
  --max-market-cap 300 --max-price 50 \
  --mode once --capital 100000
```
...
4953 只 A 股
  ...
  │   → ~300 只
  │
  ├─ 多因子排名 (动量25%+流动性20%+低波20%+趋势15%+换手10%+反转10%)
  │   → Top 300
  │
  ├─ 市值过滤 (--max-market-cap, 24h 缓存)
  │   → 60-90 只
  │
  └─ Alpha158 因子 + Ensemble 模型 + 横截面排名
      → 15-20 只持仓推荐
```
...
### Phase 2 — 已完成 ✅
- [x] **A 股市场完整接入** (Tushare + Akshare + EMT + CNPaperBroker)
- [x] **市场抽象层设计** (MarketConfig + DataProvider + Broker + Calendar)
- [x] **Risk Agent 多维升级** (6维度评分 + 个股画像 + 仓位上限 + 市场状态 + LLM叙事)
- [x] **Alpha158 因子计算器** (纯 pandas 实现, 203因子, 零 Qlib 依赖)
- [x] **Ensemble 模型** (LightGBM + RandomForest + Ridge, early stopping)
- [x] **横截面排名** (替代绝对收益预测, 去市场 beta)
- [x] **特征筛选** (LightGBM importance, 203→60)
- [x] **Portfolio 协方差优化** (风险平价 + 最小方差)
- [x] **行业中性化** (9 板块, 60+ 股票映射)
- [x] A 股全市场多因子筛选器 (硬过滤 + 六因子排名)
- [x] A 股纸交易模拟 (T+1 / 涨跌停 / 整手 / FIFO)

### Phase 3 — 已完成 ✅
- [x] **记忆系统深度整合** (Neo4j 自动召回→仓位调整, 34+ 条经验)
- [x] **News Agent** (CCTV 板块趋势 + 东方财富个股新闻 + 情感词典)
- [x] **市值过滤** (≤300亿 小盘 + 24h 缓存)
- [x] **股价过滤** (≤¥50 低价 + Screener 硬过滤)
- [x] **信号融合** (80% Alpha + 20% 新闻情绪)
- [x] **滚动周因子缓存** (52x 性能提升)
- [x] 10 个关键 bug 修复 (crash/数据错/静默失败)
- [x] ⚡ Screener 提速 75% (batch 200→500, sleep 减半)
